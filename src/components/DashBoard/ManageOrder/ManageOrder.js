...
const ManageOrder = () => {
  ...
  const handleDeleteUser = (id) => {
    ...
    if (proceed) {
      fetch(`https://agile-fjord-90292.herokuapp.com/orders/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
  ...
  const handleUpdate = (id) => {
    fetch(`https://agile-fjord-90292.herokuapp.com/orders/update/${id}`, {
      method: "PUT",
      headers: {
  ...
  useEffect(() => {
    fetch(`https://agile-fjord-90292.herokuapp.com/allorders`)
      .then((res) => res.json())
